...
const PhotoGallery: React.FC = () => {
    ...
    const hotelId = searchParams?.get('hotelId'); // Use optional chaining to safely access hotelId
    ...
    return (
        <div className="gallery-container p-5 max-w-6xl mx-auto">
            ...
            <div className="photo-gallery flex gap-2 h-96">
                <div className="main-photo flex-2 h-full">
                    <img src={hotelData.images[0]} alt={hotelData.title} className="w-full h-full object-cover rounded-lg" />
                </div>
                <div className="side-photos flex flex-col gap-2 flex-1 overflow-hidden">
                    {hotelData.images.slice(1).map((image, index) => (
                        <div className="photo-row flex gap-2 flex-1" key={index}>
                            <div className="photo relative flex-1 h-full cursor-pointer">
            ...
            {/* Additional modal and tabs logic can be added here */}
        </div>
    );
};
